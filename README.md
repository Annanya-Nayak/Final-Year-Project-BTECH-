# Adaptive Cryptographic Framework for AI Web Services

This is a FastAPI service that runs DistilBERT sentiment analysis. Instead of using one fixed encryption scheme, it uses a Double Q-learning agent to choose between classical, hybrid, and post-quantum crypto for every request. The agent looks at CPU load, latency, payload size, and the sensitivity level the client sends, and picks a mode based on that. Low-sensitivity requests don't need post-quantum overhead every time. High-sensitivity ones almost always get it.

Early on, post-quantum selection for high-sensitivity traffic was stuck at 54.5%. The cause was a single shared Q-table across all sensitivity levels, which let low-sensitivity decisions influence high-sensitivity policy. Splitting this into three separate Double Q-tables, one per sensitivity level, fixed it and raised the number to 82.8%. This was the biggest improvement in the project.

Crypto overhead is small regardless of mode, under 2.2 ms across all three at 63-byte payloads. DistilBERT inference accounts for most of the latency, not encryption.
